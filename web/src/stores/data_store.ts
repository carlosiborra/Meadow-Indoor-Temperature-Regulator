...
export const FETCH_INTERVAL = import.meta.env.FETCH_INTERVAL ?? 6000
...
export async function updateParams(
    ...
    const controller = new AbortController();
    const timeout = FETCH_INTERVAL * 5 / 6
    const id = setTimeout(() => controller.abort(), timeout);


    const start = Date.now()
    try{
        const resp = await fetch(`${BASE_URL}/setparams`, {
            method: 'POST',
            body: JSON.stringify({
                pass,
        ...
        if(resp.status !== 200){
            throw new Error(`Failed to update params. Status: ${resp.status}`)
        }
